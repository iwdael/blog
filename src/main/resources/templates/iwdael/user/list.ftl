                                ...
                                ${role.name}
                                    ...
                                    <td class="text-center">
                                        <#if row.id != 1>
                                            ...
                                            <a href="${base}/iwdael/user/pwd?id=${row.id}" class="btn btn-xs btn-success">修改密码</a>

                                            <a href="${base}/iwdael/user/view?id=${row.id}" class="btn btn-xs btn-primary">修改角色</a>
                                        <#else>
    ...
    $('#dataGrid a[data-action="close"]').bind('click', function(){
		...
		layer.confirm('该账号停用后，将不能登录系统，确定要停用?', {
            ...
            shade: false //不显示遮罩
			 ...
			 J.getJSON('${base}/iwdael/user/close', {id: that.attr('data-id'), active: false}, ajaxReload);
        }, function(){
        });
        return false;
    });
    ...
    $('#dataGrid a[data-action="open"]').bind('click', function(){
		...
		layer.confirm('该账号激活后，将可访问系统中的已授权功能，确定要激活?', {
            ...
            shade: false //不显示遮罩
			 ...
			 J.getJSON('${base}/iwdael/user/open', {id: that.attr('data-id'), active: true}, ajaxReload);
        }, function(){
        });
        return false;
    });
